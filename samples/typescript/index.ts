...
function firstPointer(result: unknown): string {
  if (typeof result !== 'object' || result === null) {
    throw new Error('Upsert returned no pointer');
  }
  const value = result as Record<string, unknown>;
  const pointers = value.Pointers ?? value.pointers;
...
async function main(): Promise<void> {
  ...
  const bays = await metadataClient.status({
    Schemas: { database_name: databaseName }
  });
  const drawers: StorageInventory[] = await metadataClient.status({
  ...
  const verifiedBook: unknown = await wardrobe.read(bookPointer);
  const verifiedAuthor: unknown = await wardrobe.read(authorPointer);
  const verifiedEditor: unknown = await wardrobe.read(editorPointer);
  ...
  const publisher: unknown = await wardrobe.read(publisherPointer);
  console.log(
  ...
  console.log(`\nWardrobe TypeScript binding ${PACKAGE_VERSION}`);
