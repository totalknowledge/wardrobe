...
The MIT-licensed `bindings/c` crate exposes the `wardrobe-c` native library at version `0.26.722` for C and C-compatible consumers.
...
`wardrobe_cabi_status_databases` returns a `WardrobeCStatus` with success and database-count fields. `wardrobe_cabi_execute_command` returns an allocated serialized `CommandResult`; callers release that string with `wardrobe_cabi_free_string`. A status command result contains the direct status payload under the protocol's `status` operation envelope.
