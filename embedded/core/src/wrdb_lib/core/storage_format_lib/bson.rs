...
pub(super) const BSON_FRAME_MAGIC: [u8; 4] = *b"WRDB";
pub(super) const BSON_FRAME_VERSION: u8 = 1;
pub(super) const BSON_NATIVE_FRAME_VERSION: u8 = 2;
pub(super) const BSON_FLAG_TOMBSTONE: u8 = 0b0000_0001;
pub(super) const BSON_FRAME_HEADER_LEN: usize = 16;
...
impl BsonBinaryFormat {
    ...
    pub(super) fn parse_frame(bytes: &[u8]) -> std::io::Result<Option<(u8, u8, usize, usize)>> {
        if !Self::is_binary_frame(bytes) {
            return Ok(None);
        }

        let version = bytes[4];
        if version != BSON_FRAME_VERSION && version != BSON_NATIVE_FRAME_VERSION {
            return Err(Error::new(
                ErrorKind::InvalidData,
        ...
        Ok(Some((version, flags, frame_len, slot_len)))
    }
}
...
impl StorageFormat for BsonBinaryFormat {
    ...
    fn deserialize_record(bytes: &[u8]) -> std::io::Result<Option<Value>> {
        Self::deserialize_record_with_map(bytes, None)
    }

    fn is_tombstone(bytes: &[u8]) -> bool {
        if let Ok(Some((_version, flags, _frame_len, _slot_len))) = Self::parse_frame(bytes) {
