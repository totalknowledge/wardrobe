...
use std::thread;
use wardrobe_embedded::CatalogRegistry;
...
fn status_storage(engine: &WardrobeEngine) -> std::io::Result<wardrobe_embedded::StorageDiagnosis> {
...
) -> std::io::Result<wardrobe_embedded::WalVerification> {
...
fn us_127_open_with_config_applies_cache_and_wal_settings_without_global_logging() {
    ...
    config.wal.durability = wardrobe_embedded::DurabilityPolicy::Grouped {
        commit_window_ms: 9,
        max_batch_size: 11,
    };
    config.logging.level = wardrobe_embedded::ApplicationLogLevel::Info;
    ...
    assert_eq!(
        ...
        wardrobe_embedded::DurabilityPolicy::Grouped {
            commit_window_ms: 9,
            max_batch_size: 11
        }
    );
